...
char *msg_gadevents_code_ccplus_checkbox[] =
{
...
char *msg_gadevents_code_ccplus_checkbox_defroutine[] =
{
	"void SetLabel(text)",
	"void SetValue(value)",
...
void
generate_header_begin_creator(FILE *fi,char *name,int nb)
{
	...
	sprintf(&buf[0],"#ifndef __%s%d",name,nb);

	xwrite_string(fi,&buf[0]);	

	xwrite_cr(fi);

	sprintf(&buf[0],"class B%s%d : public B%s",name,nb,name);

	xwrite_string(fi,&buf[0]);	

	xwrite_string(fi,"{\npublic:\n");	

	sprintf(&buf[0],"\tB%s%d(BRect rct);",name,nb);

	xwrite_string(fi,&buf[0]);	

	sprintf(&buf[0],"\t~B%s%d(void);",name,nb);
...
void
generate_header_end_creator(FILE *fi,char *name,int nb)
{
	char tmp[B_PATH_NAME_LENGTH],*fichier = &tmp[0];
	...
	sprintf(&buf[0],"#define __%s%d",name,nb);
...
void
generate_code_ccplus_gad_checkbox(char *path,struct MyGadget *gad)
{
	char tmp[B_PATH_NAME_LENGTH],*fichier = &tmp[0];
	...
	if (fi = fopen(fichier,"w+"))
	{
		...
		sprintf(&buf[0],"#include \"%s_%s.h\"",&defwin->Name[0],&gad->Name[0],count_checkbox);
		...
		sprintf(&buf[0],"BCheckBox%d::BCheckBox%d(BRect rct):BCheckBox(rct,\"%s\",\"%s\",new BMessage((uint32)%d),%s,%s)",count_checkbox,count_checkbox,&gad->Name[0],&gad->Title[0],gad->msg,GetAlign(gad),GetFlagView(gad->view));

		xwrite_string(fi,&buf[0]);

		generate_crochet_code(fi,gad);

		sprintf(&buf[0],"BCheckBox%d::~BCheckBox%d(void)\n{\n}\n",count_checkbox,count_checkbox);

		xwrite_string(fi,&buf[0]);

		gencode_gadget_all(fi,gad,"BCheckBox",count_checkbox,&msg_gadevents_code_ccplus_checkbox[0],&msg_gadevents_code_ccplus_checkbox_defroutine[0]);
...
void
gen_ccplus_gad_checkbox(char *path,struct MyGadget *gad)
{
	++count_checkbox;

	generate_header_gad_all("CheckBox",count_checkbox,path,gad,&msg_gadevents_code_ccplus_checkbox[0]);

	generate_code_ccplus_gad_checkbox(path,gad);
}
