...
char *msg_gadevents_code_ccplus_text[] =
{
...
char *msg_gadevents_code_ccplus_text_defroutine[] =
{
...
void
generate_code_ccplus_gad_text(char *path,struct MyGadget *gad)
{
	...
	if (fi = fopen(fichier,"w+"))
	{
		...
		sprintf(&buf[0],"#include \"%s_%s.h\"",&defwin->Name[0],&gad->Name[0],count_text);
		...
		sprintf(&buf[0],"BStringView%d::BStringView%d(BRect rct):BStringView(rct,\"%s\",\"%s\",%s,%s)",count_text,count_text,&gad->Name[0],&gad->Title[0],GetAlign(gad),GetFlagView(gad->view));

		xwrite_string(fi,&buf[0]);

		generate_crochet_code(fi,gad);

		sprintf(&buf[0],"BStringView%d::~BStringView%d(void)\n{\n}\n",count_text,count_text);

		xwrite_string(fi,&buf[0]);

		gencode_gadget_all(fi,gad,"BStringView",count_text,&msg_gadevents_code_ccplus_text[0],&msg_gadevents_code_ccplus_text_defroutine[0]);
...
void
gen_ccplus_gad_text(char *path,struct MyGadget *gad)
{
	++count_text;

	generate_header_gad_all("StringView",count_text,path,gad,&msg_gadevents_code_ccplus_text[0]);

	generate_code_ccplus_gad_text(path,gad);
}
