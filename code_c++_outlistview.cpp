...
char *msg_gadevents_code_ccplus_outlistview[] =
{
	"bool AddUnder(BListItem *item,BListItem *underitem)",
...
char *msg_gadevents_code_ccplus_outlistview_defroutine[] =
{
	"bool AddUnder(item,underitem)",
	"bool AddItem(item)",
...
void
generate_code_ccplus_gad_outlistview(char *path,struct MyGadget *gad)
{
	...
	if (fi = fopen(fichier,"w+"))
	{
		...
		sprintf(&buf[0],"#include \"%s_%s.h\"",&defwin->Name[0],&gad->Name[0],count_outlistview);
		...
		sprintf(&buf[0],"BOutlineListView%d::BOutlineListView%d(BRect rct):BOutlineListView(rct,\"%s\",%s,%s,%s)",count_outlistview,count_outlistview,&gad->Name[0],selection_lv[gad->data.olv.multi],GetAlign(gad),GetFlagView(gad->view));

		xwrite_string(fi,&buf[0]);

		generate_crochet_code(fi,gad);

		sprintf(&buf[0],"BOutlineListView%d::~BOutlineListView%d(void)\n{\n}\n",count_outlistview,count_outlistview);

		xwrite_string(fi,&buf[0]);

		gencode_gadget_all(fi,gad,"BOutlineListView",count_outlistview,&msg_gadevents_code_ccplus_outlistview[0],&msg_gadevents_code_ccplus_outlistview_defroutine[0]);
...
void
gen_ccplus_gad_outlistview(char *path,struct MyGadget *gad)
{
	++count_outlistview;

	generate_header_gad_all("OutlineListView",count_outlistview,path,gad,&msg_gadevents_code_ccplus_outlistview[0]);

	generate_code_ccplus_gad_outlistview(path,gad);
}
