...
char *msg_gadevents_code_ccplus_view[] =
{
...
char *msg_gadevents_code_ccplus_view_defroutine[] =
{
...
void
generate_code_ccplus_gad_view(char *path,struct MyGadget *gad)
{
	...
	if (fi = fopen(fichier,"w+"))
	{
		...
		sprintf(&buf[0],"#include \"%s_%s.h\"",&defwin->Name[0],&gad->Name[0],count_view);
		...
		sprintf(&buf[0],"BView%d::BView%d(BRect rct):BView(rct,\"%s\",%s,%s)",count_view,count_view,&gad->Name[0],GetAlign(gad),GetFlagView(gad->view));

		xwrite_string(fi,&buf[0]);

		generate_crochet_code(fi,gad);

		sprintf(&buf[0],"BView%d::~BView%d(void)\n{\n}\n",count_view,count_view);

		xwrite_string(fi,&buf[0]);

		gencode_gadget_all(fi,gad,"BView",count_view,&msg_gadevents_code_ccplus_view[0],&msg_gadevents_code_ccplus_view_defroutine[0]);
...
void
gen_ccplus_gad_view(char *path,struct MyGadget *gad)
{
	++count_view;

	generate_header_gad_all("View",count_view,path,gad,&msg_gadevents_code_ccplus_view[0]);

	generate_code_ccplus_gad_view(path,gad);
}
