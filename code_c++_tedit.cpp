...
char *msg_gadevents_code_ccplus_tedit[] =
{
...
char *msg_gadevents_code_ccplus_tedit_defroutine[] =
{
	"bool AcceptsDrop(message)",
	"bool AcceptsPaste(clipboard)",
	"bool CanEndLine(offset)",
	"void FindWord(offset,start,finish)",
	"void Select(startOffset,endOffset)",
	"void ScrollToOffset(inOffset)",
	"void Undo(clipboard)",
...
void
generate_code_ccplus_gad_tedit(char *path,struct MyGadget *gad)
{
	...
	if (fi = fopen(fichier,"w+"))
	{
		...
		sprintf(&buf[0],"#include \"%s_%s.h\"",&defwin->Name[0],&gad->Name[0],count_tedit);
		...
		sprintf(&buf[0],"BTextView%d::BTextView%d(BRect rct):BTextView(rct,\"%s\",rct,%s,%s)",count_tedit,count_tedit,&gad->Name[0],GetAlign(gad),GetFlagView(gad->view));

		xwrite_string(fi,&buf[0]);

		generate_crochet_code(fi,gad);

		sprintf(&buf[0],"BTextView%d::~BTextView%d(void)\n{\n}\n",count_tedit,count_tedit);

		xwrite_string(fi,&buf[0]);

		gencode_gadget_all(fi,gad,"BTextView",count_tedit,&msg_gadevents_code_ccplus_tedit[0],&msg_gadevents_code_ccplus_tedit_defroutine[0]);
...
void
gen_ccplus_gad_tedit(char *path,struct MyGadget *gad)
{
	++count_tedit;

	generate_header_gad_all("TextView",count_tedit,path,gad,&msg_gadevents_code_ccplus_tedit[0]);

	generate_code_ccplus_gad_tedit(path,gad);
}
