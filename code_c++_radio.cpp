...
char *msg_gadevents_code_ccplus_radio[] =
{
...
char *msg_gadevents_code_ccplus_radio_defroutine[] =
{
	"void SetLabel(text)",
	"void SetValue(value)",
...
void
generate_code_ccplus_gad_radio(char *path,struct MyGadget *gad)
{
	...
	if (fi = fopen(fichier,"w+"))
	{
		...
		sprintf(&buf[0],"#include \"%s_%s.h\"",&defwin->Name[0],&gad->Name[0],count_radio);
		...
		sprintf(&buf[0],"BRadioButton%d::BRadioButton%d(BRect rct):BRadioButton(rct,\"%s\",\"%s\",new BMessage((uint32)%d),%s,%s)",count_radio,count_radio,&gad->Name[0],&gad->Title[0],gad->msg,GetAlign(gad),GetFlagView(gad->view));

		xwrite_string(fi,&buf[0]);

		generate_crochet_code(fi,gad);

		sprintf(&buf[0],"BRadioButton%d::~BRadioButton%d(void)\n{\n}\n",count_radio,count_radio);

		xwrite_string(fi,&buf[0]);

		gencode_gadget_all(fi,gad,"BRadioButton",count_radio,&msg_gadevents_code_ccplus_radio[0],&msg_gadevents_code_ccplus_radio_defroutine[0]);
...
void
gen_ccplus_gad_radio(char *path,struct MyGadget *gad)
{
	++count_radio;

	generate_header_gad_all("RadioButton",count_radio,path,gad,&msg_gadevents_code_ccplus_radio[0]);

	generate_code_ccplus_gad_radio(path,gad);
}
